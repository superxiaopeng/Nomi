...
import { CONTROL_ICON_BUTTON_CLASS, TEXT_CLIP_MIN_SEC, TEXT_CLIP_STEP_SEC } from './previewControlTokens'
...
// 预览控制条：选中字幕/标题 clip 时的样式控件（字号 + 字体 + 停留时长）。
...
export function TextClipStyleControls({ timeline, selectedTextClipId }: Props): JSX.Element | null {
  ...
  const [durationSecDraft, setDurationSecDraft] = React.useState('')
  ...
  const clipFps = timeline.fps || 30
  const selectedDurationSec = selectedTextClip
    ? Math.round(((selectedTextClip.endFrame - selectedTextClip.startFrame) / clipFps) * 10) / 10
    : 0

  // 输入框：未聚焦跟随真实值，聚焦编辑用本地 draft。
  React.useEffect(() => { setSizePctDraft(String(selectedSizePct)) }, [selectedSizePct, selectedTextClipId])
  React.useEffect(() => { setDurationSecDraft(selectedDurationSec.toFixed(1)) }, [selectedDurationSec, selectedTextClipId])
  ...
  // 改时长 = 钉住起点、移右边缘（resizeTextClip 已保证 ≥1 帧）。
  const applyDurationSec = React.useCallback((seconds: number) => {
    if (!selectedTextClip) return
    const safe = Math.max(TEXT_CLIP_MIN_SEC, seconds)
    const nextEnd = selectedTextClip.startFrame + Math.max(1, Math.round(safe * clipFps))
    resizeTimelineTextClip(selectedTextClip.id, 'right', nextEnd, { commit: true })
  }, [selectedTextClip, clipFps, resizeTimelineTextClip])

  const commitDurationSec = React.useCallback(() => {
    const seconds = Number(durationSecDraft)
    if (Number.isFinite(seconds) && seconds > 0) applyDurationSec(seconds)
  }, [applyDurationSec, durationSecDraft])
  ...
  return (
    <>
      ...
      <div className={cn('workbench-preview-player__text-style', 'flex-none inline-flex items-center gap-1.5')} aria-label="文字样式">
        ...
        />
        <span className="text-[11px] text-[var(--workbench-muted)] font-bold">时长</span>
        <div className="inline-flex items-center gap-[3px]">
          <WorkbenchIconButton className={cn(CONTROL_ICON_BUTTON_CLASS)} label="缩短时长" icon={<IconMinus size={14} />} onClick={() => applyDurationSec(selectedDurationSec - TEXT_CLIP_STEP_SEC)} />
          <input
            className={cn('w-[44px] h-6 text-center text-[11px] font-bold tabular-nums', 'rounded-[var(--nomi-radius-sm)] border border-[var(--workbench-border)] bg-[var(--nomi-paper)] text-[var(--workbench-ink)] outline-none focus:border-[var(--nomi-accent)]')}
            value={durationSecDraft}
            inputMode="decimal"
            aria-label="停留时长秒"
            onChange={(event) => setDurationSecDraft(event.target.value.replace(/[^0-9.]/g, ''))}
            onBlur={commitDurationSec}
            onKeyDown={(event) => { if (event.key === 'Enter') (event.target as HTMLInputElement).blur() }}
          />
          <span className="text-[11px] text-[var(--workbench-muted-soft)]">秒</span>
          <WorkbenchIconButton className={cn(CONTROL_ICON_BUTTON_CLASS)} label="延长时长" icon={<IconPlus size={14} />} onClick={() => applyDurationSec(selectedDurationSec + TEXT_CLIP_STEP_SEC)} />
        </div>
      </div>
    </>
  )
