...
import { normalizeDimensionScore } from "./judge.mjs";
...
export async function scoreAssetWithVlm(cfg, { kind, prompt, images, model }) {
  ...
  const res = await fetch(`${String(cfg.baseUrl).replace(/\/$/, "")}/chat/completions`, {
    ...
    body: JSON.stringify({ model: model || cfg.visionModel || cfg.model, temperature: 0, response_format: { type: "json_object" }, messages: [{ role: "user", content }] }),
  });
