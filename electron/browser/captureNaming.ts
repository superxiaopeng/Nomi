...
export function captureFileName(
  mediaUrl: string,
  contentType: string,
  kindHint: "image" | "video" | "screenshot",
  preferredName?: unknown,
): string {
  const ext = extensionFromMime(contentType, kindHint === "video" ? "mp4" : "png");
  let base = captureNameStem(preferredName);
  if (/^https?:\/\//i.test(mediaUrl)) {
    try {
      const pathname = new URL(mediaUrl).pathname;
      const last = pathname.split("/").filter(Boolean).pop() || "";
      if (!base) base = captureNameStem(last);
    } catch {
      // URL 解析失败 → 走兜底名
    }
  }
...
function captureNameStem(value: unknown): string {
  const raw = String(value || "").trim().split(/[\\/]/).pop() || "";
  // 先解码再验证（%2F 解码后才现形为 /），拒含 .. 的名——防目录穿越进文件名。
  const stem = decodeURIComponentSafe(raw.replace(/\.[a-z0-9]{1,8}$/i, "").slice(0, 60));
  if (!/^[\w\-. ()一-鿿]+$/u.test(stem) || !stem.trim() || stem.includes("..")) return "";
  return stem;
}
