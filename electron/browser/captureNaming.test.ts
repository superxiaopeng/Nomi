...
describe("captureFileName", () => {
  ...
  it("keeps a readable preferred title but always replaces its extension from verified bytes", () => {
    expect(captureFileName("https://cdn.a.com/file", "video/mp4", "video", "launch.clip.jpg")).toBe("launch.clip.mp4");
    expect(captureFileName("https://cdn.a.com/file", "image/png", "image", "v1.2")).toBe("v1.png");
  });
  ...
  it("uses the shared media registry for browser-native image and video formats", () => {
    expect(captureFileName("https://a.com/asset", "image/avif", "image")).toBe("asset.avif");
    expect(captureFileName("https://a.com/asset", "video/quicktime", "video")).toBe("asset.mov");
  });
