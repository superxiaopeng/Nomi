...
7. **用户可见改动？** **先读设计系统 `docs/design/nomi-design-system.md`（token/组件/规范）再画**，再出样张——**有交互的默认出能亲手拖/点的可体验 widget（show_widget），不只静态图，别等用户要**；token-only（禁非 token 的 px/hex）；渐进展开 / 防遮挡 / 空间极简。（→ R8）
