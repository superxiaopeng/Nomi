...
> 本文件按需查阅，不要求每次 session 全读。CLAUDE.md 中的规则索引触发对应编号后来这里查细节。
...
**验证门槛**：`pnpm build` 绿 + `npx vitest run` 不回归。

**commit 规范**：
- 一个逻辑改动一个 commit
- Message：做了什么 + 为什么 + 验证结果，中文风格，结尾 `Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>`
