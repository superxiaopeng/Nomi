...
|---|---|---|---|
| `--nomi-font-sans` | `font-nomi-sans`（默认）| `Inter, -apple-system, BlinkMacSystemFont, "PingFang SC", "Hiragino Sans GB", system-ui, sans-serif` | 正文、UI 全局 |
| `--nomi-font-display` | **`font-nomi-display`** | `Fraunces, Inter, serif` | 品牌字标「Nomi」、大型显示标题、有"数字/编辑感"的标题 |
| `--nomi-font-mono` | `font-nomi-mono` | `ui-monospace, SFMono-Regular, "SF Mono", Menlo, …` | 代码块、等宽数字 |

**用法铁律：要显示字体永远用类名 `font-nomi-display` / `font-nomi-sans`，不要内联写 `font-[Fraunces,Inter,serif]` 或 `font-[var(--nomi-font-display)]`**——那是绕过 token、各处字栈会漂的写法（已知 3 处违规见 §14）。

> ⚠️ **真实缺口（v2 实测）：Inter 与 Fraunces 都没有被打包/自托管**——仓库里没有任何 `.woff/.ttf` 字体文件，也没装 `@fontsource/*`。字体栈靠**系统是否装了**这两个字体来渲染。后果：
> - **Fraunces（品牌显示字）若用户机器没装，会回退到 Inter→serif**——即字标「Nomi」在干净 macOS/Windows 上很可能不是设计里的 Fraunces 衬线，而是系统衬线。
> - 这是品牌一致性的隐患（logo 字标的视觉依赖一个没保证存在的字体）。
> - 修法（待拍板，§14 backlog）：装 `@fontsource-variable/fraunces` + `@fontsource-variable/inter` 自托管，确保离线/任意机器一致。本地优先产品尤其该自带字体。
...
| # | 现状 | 为什么是问题 | 修法 |
|---|---|---|---|
| S1 | `globals.css` 默认 `:root` 是一整套**暗色 `--tc-*` token**（`--tc-color-app-bg: #05070b`、body 默认暗色渐变背景），「光模式」只是 `[data-mantine-color-scheme="light"]` 覆盖层 | 与「light-only」原则直接矛盾；一个休眠的旧暗色主题躺在仓库里，`--tc-*` 全是 hex，是 §0.5 ③ 层包袱。当前仅 ~5 个 `--tc-*` 还被消费 | 把残留 5 处迁到 `--nomi-*`，删整段 `--tc-*` 暗色层 + body 暗色渐变，`color-scheme` 改 `light` |
| S2 | `globals.css` `color-scheme: light dark` | 声明支持暗色，与 light-only 矛盾，可能让原生控件（滚动条/表单）出暗色 | 改成 `color-scheme: light` |
| S3 | Inter / Fraunces **未打包**，靠系统字体 | 品牌字标 Fraunces 在干净机器上回退，logo 字标视觉无保证（详见 §2.6）| 装 `@fontsource-variable/{fraunces,inter}` 自托管 |
| S4 | `--handle-color-*`（连线句柄色，globals.css）定义了但 **src 内 0 个 `var()` 消费点** | 疑似死 token 或连线实际另有取色路径，真相源不清 | 核实画布连线实际取色 → 要么接上要么删 |
| S5 | 语义色 `--workbench-success/danger/video` 等是 hex/rgba（workbench.css），非 oklch | 与「颜色只在 nomi-tokens.css 写 oklch」不齐（§2.1.4）| 回收进 `--nomi-*` oklch 体系 |
