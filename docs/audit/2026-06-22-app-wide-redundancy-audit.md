# 全 App UX 冗余/复杂度审计（从头到脚）

> 2026-06-22 ｜ 用户：「扫描出类似的…从头到脚优化一遍」。五路并行只读体检（创作 / 画布 / 预览 / 库+跨切面 / 导航），用「模型接入」那把尺子（入口重叠 / 两套心智-并行版 / 概念过载 / 字段没收起 / 分散入口 / 长尾占主路 / 文案不一致）。本文档=现状地图，**只诊断**；改法逐项出样张拍板。
